...
tools/htkbin/HLEd -A -D -V -T 1 -n hmm0/hmms.mlist -i train.mlf $BIPHONE_PATH/make_xwbi-rc.led lib/mlabs/train.mlf > hmm0/LOG.hled
echo 'CL hmm0/hmms.mlist' > hmm0/clone_xwbi-rc.hed
#     generate the untied triphone HMMs
tools/htkbin/HHEd -A -D -V -T 1 -H hmm0/work/MMF -M hmm0 hmm0/clone_xwbi-rc.hed hmm0/work/hmms.mlist > hmm0/LOG.hhed
...
cat lib/edfiles/ROVAL_TBVAL.hed | sed "s=ROVAL=$ROVAL=g; s=TBVAL=$TBVAL=g; s=TREENAME=hmm10/$TREENAME=g; s=LISTNAME=hmm10/$LISTNAME=g; s=BIPHONEPATH=$BIPHONE_PATH=g" > hmm10/$HEDNAME
